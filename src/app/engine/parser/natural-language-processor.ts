...
export class NaturalLanguageProcessor {
  ...
  public getTerms(sentence: string): LanguageTerm[] {
    ...
    if (this.shouldLog) {
      LoggingService.instance.log(`Language service is extracting terms from: ${sentence}`);
    }
  ...
  public static getTokenFromTerm(term): CommandToken {
    ...
    token.classification = NaturalLanguageProcessor.getClassificationFromTerm(term);
    return token;
  }
  ...
  public getTokenForWord(word: string): CommandToken {
    ...
    return NaturalLanguageProcessor.getTokenFromTerm(terms[0]);
  }
  ...
  public getTokensForSentence(sentence: string): CommandToken[] {
    ...
    for (const term of terms) {
      ...
      const token = NaturalLanguageProcessor.getTokenFromTerm(term);
      ...
      LoggingService.instance.log(token);
      tokens.push(token);
